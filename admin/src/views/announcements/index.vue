...
const announcements = ref([
  ...
  { id: 2, title: '新功能上线', audience: 'learners', status: 'published', publishedAt: '2024-01-14', expiresAt: '2024-02-14' },
  { id: 3, title: '管理员会议', audience: 'admins', status: 'draft', publishedAt: '', expiresAt: '' },
])
        ...
        <el-table :data="announcements" style="width: 100%">
          ...
          <el-table-column prop="audience" label="面向对象">
            <template #default="{ row }">
              <el-tag :type="row.audience === 'all' ? 'primary' : row.audience === 'learners' ? 'success' : 'warning'">
                {{ row.audience === 'all' ? '全部用户' : row.audience === 'learners' ? '全部学员' : '全部管理员' }}
              </el-tag>
            </template>
          </el-table-column>
      ...
      <el-form v-if="editingAnnouncement" :model="editingAnnouncement" label-width="100px">
        ...
        <el-form-item label="面向对象">
          <el-select v-model="editingAnnouncement.audience">
            ...
            <el-option label="全部学员" value="learners" />
            <el-option label="全部管理员" value="admins" />
          </el-select>
        </el-form-item>
