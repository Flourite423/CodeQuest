...
#[handler]
pub async fn list_social_activities(depot: &mut Depot) -> Result<Json<ApiResponse<Vec<SocialActivity>>>, StatusError> {
    let pool = depot.obtain::<PgPool>()
        .map_err(|_| StatusError::internal_server_error())?;

    let learner_id = auth::get_current_account_id(depot)?;
    let activities = sqlx::query_as::<_, SocialActivity>(
        "SELECT * FROM social_activities WHERE learner_id = $1 ORDER BY created_at DESC LIMIT 50"
    )
    .bind(learner_id)
    .fetch_all(pool)
    .await
